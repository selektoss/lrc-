...
void GetDataResourceMenu(ModelOBJ*& form, char*& title_contetnt, char**& nameSection)
{
	...
	if (HRSRC FileResource = FindResource(GetModuleHandle(NULL), MAKEINTRESOURCE(INITXTFILE), "ini"))
	{
		...
		if (lineBuff.good())
			...
			{
				if (regex_match(lineTemp.c_str(), res, regex{ "^(#\\HEADER[\\s\\S]|#\\MENU[\\s\\S]|#\\HELP[\\s\\S]|#\\TITLE[\\s\\S]|#\\CONTENT[\\s\\S]*)" }, std::regex_constants::match_not_eol))
				{
					res[0] == regular[0] ? check = 1 : res[0] == regular[2] ? check = 3 : res[0] == regular[4] ? check = 5 : res[0] == regular[8] ? check = 9 : check = 7;
					while (getline(lineBuff, lineTemp) && !(regex_match(lineTemp.c_str(), regex{ *(regular + check) })))
					{
					...
					if (count && check != 9)
					{
						...
						for (uint16_t iter(0); iter < count; ++iter)
							check == 3 ? getline(lineBuffM, ((form->Menu) + iter)->nameButton) : getline(lineBuffM, ((form->Menu) + iter)->txtHelp);
		...
		else
		{
			...
			SetCurPos(static_cast<uint16_t>((form->coordXY.X >> 1) - ((strlen(info_error)) >> 1)), static_cast<uint16_t>(form->coordXY.Y >> 1));
			std::cout << info_error;
			Sleep(2000); exit(-2);
		}
	}
